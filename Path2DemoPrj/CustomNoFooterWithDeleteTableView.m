//
//  CustomNoFooterWithDeleteTableView.m
//  虎踞龙蟠
//
...
@implementation CustomNoFooterWithDeleteTableView
...
- (void)refreshTableHeaderDidTriggerRefresh:(RefreshTableHeaderView*)view
{
    ...
    {
        [mDelegate refreshTableHeaderDidTriggerRefresh:view];
    }
}
...
- (void)refreshTableFooterDidTriggerRefresh:(RefreshTableFooterView*)view
{
    ...
    if([mDelegate respondsToSelector:@selector(refreshTableFooterDidTriggerRefresh:)])
        [mDelegate refreshTableFooterDidTriggerRefresh:view];
}
...
{
    if([mDelegate respondsToSelector:@selector(tableView:commitEditingStyle:forRowAtIndexPath:)])
        [mDelegate tableView:tableView commitEditingStyle:editingStyle forRowAtIndexPath:indexPath];
    else
        return;
}
...
{
    if([mDelegate respondsToSelector:@selector(tableView:editingStyleForRowAtIndexPath:)])
        return [mDelegate tableView:tableView editingStyleForRowAtIndexPath:indexPath];
    else
