...
- (void)updateDisplayWithCell:(UITableViewCell *)cell {
    ...
    if (dateComponents.year == todayComponents.year && dateComponents.month == todayComponents.month && dateComponents.day == todayComponents.day) {
        ...
        NSArray * array = [NSArray arrayWithObjects:@"开始", @"一", @"二", @"三", @"四", @"五", @"六", @"天", nil];
        dateLabelString = [NSString stringWithFormat:@"星期%@", [array objectAtIndex:dateComponents.weekday]];
        [dateFormatter release];
        dateLabelAlpha = 1.0f;
        ...
        timeLabelFrame = CGRectMake(30.0f, 4.0f, 100.0f, 10.0f);
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        dateFormatter.dateFormat = @"M月d日, yyyy";
        dateLabelString = [dateFormatter stringFromDate:date];
        [dateFormatter release];
        dateLabelAlpha = 1.0f;
